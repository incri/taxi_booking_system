...
class DriverLogInPage:
    ...
    def create_driver_login_frame(self):
        ...
        self.build_driver_login_frame(login_frame=self.login_frame, root=self.root)

    @staticmethod
    def build_driver_login_frame(login_frame, root):
        ...
        login_button.configure(
            ...
            foreground="#FFFFFF",
            text="""Log In""",
            command=lambda: DriverLogInPage.admin_login(
                username_entry,
                password_entry,
                login_frame,
                root,
        ...
        show_pass_button.bind(
            "<Button-1>",
            lambda event: DriverLogInPage.show_pass(
                event,
                password_entry,
            ),
        )
        ...
        show_pass_button.bind(
            "<ButtonRelease-1>",
            lambda event: DriverLogInPage.hide_pass(
                event,
                password_entry,
            ),
        )
    ...
    def admin_login(
        ...
        user_control = DriverController()
        user_control.driver_login_control(driver, login_frame, root)
